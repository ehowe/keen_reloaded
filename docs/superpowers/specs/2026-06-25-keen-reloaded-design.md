...
Implementation will be detailed in a separate plan, but the intended ordering:

1. **Godot project init** — project config, directory scaffold, git, GUT setup.
2. **Data model** — `LevelData`, `LevelPack`, `EntityDef` resources; serialization tests.
3. **Editor MVP** — 3-panel UI, tile painting, entity placement, save/load `.tres`, Test ▶.
4. **Runtime core** — `LevelRuntime` build-from-data, player (`CharacterBody2D`), base entity classes.
5. **Keen 1 content** — tileset, first entities (player, vorticon, items), first level via editor.
6. **Pack loading** — `PackLoader`, bundled + drop-in, level-select menu.
7. **Polish** — audio, background/parallax, HUD, save/progression, gamepad mapping.
